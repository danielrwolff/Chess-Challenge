...
public class MyBot : IChessBot
{
    ...
    int Search(int depth, int ply, int alpha, int beta) {
        ...
        if (ply > 0 && ttEntry.key == key && ttEntry.depth >= depth) {
            //Debug(ply, "found TT entry flag=" + ttEntry.flag + " with score = " + ttEntry.score);
            ...
            if (ttEntry.flag == 0
                ...
                //Debug(ply, "tt match with score=" + ttEntry.score + " for " + ttEntry.move);
    ...
    int GetPieceValue(PieceType type, Square square, Boolean isWhite, ulong phase = 12) {
        ...
        ulong mgScore = (((mgRank >> (square.File * 8)) & 255) - 127) * 2 + mg_piece_value[type_index];
        ulong egRank = eg_pesto_values[type_index - 1, Math.Abs(square.Rank - (isWhite ? 0 : 7))];
        ulong egScore = (((egRank >> (square.File * 8)) & 255) - 127) * 2 + eg_piece_value[type_index];
